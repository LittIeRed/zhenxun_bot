...
__plugin_configs__ = {
    "MAX_MONEY_BASICS": {"value": 3000, "help": "银行存款基础上限", "default_value": 3000},
    "MAX_MONEY_MULTIPLIER": {"value": 450, "help": "银行存款金额上限倍率，总上限=基础上限+倍率*好感度", "default_value": 450}}
...
async def _(event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    if CanSave >= num:
        ...
        await BagUser.spend_gold(uid, group, CanSave)
        charges = int(CanSave * 0.03) if int(CanSave * 0.03) > 1 else 1
        await TZtreasury.update_treasury_info(group, charges)
        await TZBank.update_bank_info(uid, group, CanSave - charges)
        CanSave_Int = int(CanSave)
        await save.finish(f"存入金额超过银行储蓄上限\n已自动存入当前最大可存入金额{CanSave_Int}\n{NICKNAME}收取了3%({charges})的手续费")
...
# 计算最大值
async def calculation_max_money(user_id: int, group_id: int):
    try:
        q = await SignGroupUser().get_or_none(user_id=user_id, group_id=group_id)
