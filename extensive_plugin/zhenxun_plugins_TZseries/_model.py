...
class TZBlack(Model):
    ...
    # 获取下次洗白时间
    @classmethod
    async def next_xb_Time(cls, uid, gid):
        if gid == None:
           return 0
        user = await cls.filter(uid=uid, gid=gid, state=1, wrtime__not_isnull=True).first()
        if user:
            new_time = user.wrtime + datetime.timedelta(hours=18)
            new_time_eastern = new_time.astimezone(pytz.timezone('Asia/Shanghai'))
            return new_time_eastern.strftime('%m-%d %H:%M:%S')
        return ""
