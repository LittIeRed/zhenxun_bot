...
async def _(bot:Bot, event: GroupMessageEvent):
    ...
    print(present)
    new_time = user.checkin_time_last + timedelta(hours=4)
    new_time_eastern = new_time.astimezone(pytz.timezone('Asia/Shanghai'))
    message = f"下次可打工时间在 {new_time_eastern.strftime('%m-%d %H:%M:%S')} 之后"
    if new_time_eastern >= present:
        await work.finish(message, at_sender=True)
        return
