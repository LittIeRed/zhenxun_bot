...
async def _(event: GroupMessageEvent):
    ...
    if My24H_isBlock == 0:
        await xb.finish("你没有需要洗白的黑钱", at_sender=True)

    if await TZBlack.before_Time_Has(uid, gid):
        new_time_eastern = await TZBlack.next_xb_Time(uid, gid)
        message = "下次可洗白时间在" + new_time_eastern + "之后"
        await xb.finish(message, at_sender=True)
